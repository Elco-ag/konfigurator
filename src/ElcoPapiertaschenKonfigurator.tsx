...
// ============================================================================
// MAIN COMPONENT - Always Expanded Version
// ============================================================================
  ...
  useEffect(() => {
    ...
    if (qty) {
      if (isNaN(num) || num < 50) {
        ...
        setQtyError('Bitte in 50er Schritten (50, 100, 150, 200, ...)');
      } else {
        setQtyError('');
      }
    } else {
...
${firstName} ${lastName}`);
    ...
    window.location.href = `mailto:mike.sohre@myelco.ch?subject=${subject}&body=${body}`;
      ...
      <div style={{padding: '16px', maxWidth: '800px', margin: 0}}>
        ...
        <div className="mb-8">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
            ...
            {/* 2. Farbe - Always visible */}
            <div className={`p-6 border-b border-gray-100 transition-opacity ${!handle ? 'opacity-40 pointer-events-none' : ''}`}>
              <h3 className="text-sm text-gray-500 uppercase tracking-wider mb-4" style={{fontWeight: 700}}>
                2. Farbe {!handle && <span className="text-xs normal-case">(Bitte zuerst Henkelform wählen)</span>}
              </h3>
              <div className="grid grid-cols-2 gap-4">
                <button
            ...
            {/* 3. Format - Always visible */}
            <div className={`p-6 border-b border-gray-100 transition-opacity ${!color ? 'opacity-40 pointer-events-none' : ''}`}>
              <h3 className="text-sm text-gray-500 uppercase tracking-wider mb-4" style={{fontWeight: 700}}>
                3. Format & Grösse {!color && <span className="text-xs normal-case">(Bitte zuerst Farbe wählen)</span>}
              </h3>
              <select
                value={format || ''}
            ...
            {/* 4. Druckart - Always visible */}
            <div className={`p-6 border-b border-gray-100 transition-opacity ${!format ? 'opacity-40 pointer-events-none' : ''}`}>
              <h3 className="text-sm text-gray-500 uppercase tracking-wider mb-4" style={{fontWeight: 700}}>
                4. Druckart {!format && <span className="text-xs normal-case">(Bitte zuerst Format wählen)</span>}
              </h3>
              <div className="grid grid-cols-2 gap-4">
                <button
            ...
            {/* 5. Menge - Always visible */}
            <div className={`p-6 transition-opacity ${!print ? 'opacity-40 pointer-events-none' : ''}`}>
              <h3 className="text-sm text-gray-500 uppercase tracking-wider mb-4" style={{fontWeight: 700}}>
                5. Menge {!print && <span className="text-xs normal-case">(Bitte zuerst Druckart wählen)</span>}
              </h3>
              <div className="flex flex-wrap gap-2 mb-4">
                {[50, 100, 200, 500, 1000, 2000].map(tier => (
                  <button
                    key={tier}
                    onClick={() => print && handleQuickSelection(tier)}
                    disabled={!print}
              ...
              />
              {!qtyError && !qty && (
                <p className="mt-2 text-xs text-gray-500" style={{fontWeight: 400}}>
                  💡 Eingabe in 50er Schritten: 50, 100, 150, 200, 250, usw.
                </p>
              )}
              {qtyError && (
              ...
              {qty && !qtyError && parseInt(qty) % 100 !== 0 && (
                <p className="mt-2 text-xs text-blue-600 flex items-center" style={{fontWeight: 400}}>
                  ✓ Gültige Menge
                </p>
              )}
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-blue-800 mt-1" style={{fontWeight: 400}}>
                        ...
                        </a> • <a href="tel:+41564628000" className="underline hover:text-blue-900">
                          +41 56 462 80 00
                        </a>
                      </p>
                    </div>
            ...
            <div className="p-6">
              ...
              {isPriceAvailable ? (
                <>
                  ...
                  <div className="space-y-3 pb-6 border-b border-gray-100 bg-gray-50 rounded-lg p-4">
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600" style={{fontWeight: 400}}>Konfiguration</span>
                      <span className="text-gray-900" style={{fontWeight: 700}}>
                        {handle === 'flachhenkel' ? 'Flach' : 'Kordel'} • {color === 'braun' ? 'Braun' : 'Weiss'} • {print}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600" style={{fontWeight: 400}}>Format</span>
                      <span className="text-gray-900" style={{fontWeight: 700}}>{format?.replace(/x/g, '×')} cm</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600" style={{fontWeight: 400}}>Menge</span>
                  ...
                  <div className="pt-6 border-t-2 border-gray-100">
                    <div className="bg-gradient-to-r from-gray-50 to-green-50 rounded-xl p-4">
                      <div className="flex justify-between items-baseline">
                        <span className="text-gray-700" style={{fontWeight: 700}}>Ihr Gesamtpreis</span>
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="text-gray-600 mb-2" style={{fontWeight: 700}}>
                    Bitte Konfiguration abschliessen
                  </p>
                  <p className="text-sm text-gray-500" style={{fontWeight: 400}}>
                    Wählen Sie alle Optionen aus, um den Preis zu berechnen
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg border-2 p-6" style={{borderColor: isPriceAvailable ? ELCO_RED : '#d1d5db'}}>
            ...
            <div className="space-y-4">
              ...
              <input
                type="text"
                value={company}
                onChange={(e) => isPriceAvailable && setCompany(e.target.value)}
                disabled={!isPriceAvailable}
              ...
              <div className="grid grid-cols-2 gap-4">
                <input
                  type="text"
                  value={firstName}
                  onChange={(e) => isPriceAvailable && setFirstName(e.target.value)}
                  disabled={!isPriceAvailable}
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-gray-400 focus:bg-white transition-all disabled:cursor-not-allowed"
                  placeholder="Vorname *"
                  style={{fontWeight: 400}}
                ...
                <input
                  type="text"
                  value={lastName}
                  onChange={(e) => isPriceAvailable && setLastName(e.target.value)}
                  disabled={!isPriceAvailable}
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-gray-400 focus:bg-white transition-all disabled:cursor-not-allowed"
              ...
              <input
                type="email"
                value={email}
                onChange={(e) => isPriceAvailable && setEmail(e.target.value)}
                disabled={!isPriceAvailable}
                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-gray-400 focus:bg-white transition-all disabled:cursor-not-allowed"
                placeholder="E-Mail *"
                style={{fontWeight: 400}}
              ...
              <input
                type="tel"
                value={phone}
                onChange={(e) => isPriceAvailable && setPhone(e.target.value)}
                disabled={!isPriceAvailable}
                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-gray-400 focus:bg-white transition-all disabled:cursor-not-allowed"
                placeholder="Telefon *"
                style={{fontWeight: 400}}
